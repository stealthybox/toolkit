...
Make sure you're using the `libgit2` provider, compatible with Git Protocol v2, for any `GitRepository` object
fetching from or updating Azure Repos.
...
Your AKS cluster's configuration can also be updated to allow the kubelets to pull images from ACR without ImagePullSecrets
as an optional, complimentary step.
